...
class PassportPhotoProcessor:
    ...
    def process_to_passport_photo(self, image_path, face_bbox=None, remove_bg=False, remove_watermark=False):
        ...
        if face_bbox and face_bbox.get('x') is not None and self.ENABLE_INTELLIGENT_CROPPING:
            ...
            print(f"🔍 DEBUG: Input face_bbox: {face_bbox}")
            ...
            print(f"🔍 DEBUG: Face size {fw}x{fh}, required crop size: {required_crop_width}x{required_crop_height}")
            ...
            # (was 60%, now 50% to be more conservative)
            ...
            print(f"🔍 DEBUG: Estimated full head height: {estimated_full_head_height}, required crop size: {required_crop_width}x{required_crop_height}")
            ...
            # (was 50%, now 60% to give more headroom)
            ...
            print(f"🔍 DEBUG: Initial crop_top: {crop_top}, crop_bottom: {crop_bottom}")
            ...
            if crop_bottom > img.height:
                ...
                print(f"🔍 DEBUG: Adjusted for bottom boundary: crop_top: {crop_top}, crop_bottom: {crop_bottom}")
            
            if crop_top < 0:
                crop_top = 0
                crop_bottom = min(img.height, int(required_crop_height))
                print(f"🔍 DEBUG: Adjusted for top boundary: crop_top: {crop_top}, crop_bottom: {crop_bottom}")
            ...
            print(f"🔍 DEBUG: Initial crop_left: {crop_left}, crop_right: {crop_right}")
            ...
            if crop_right > img.width:
                ...
                print(f"🔍 DEBUG: Adjusted for right boundary: crop_left: {crop_left}, crop_right: {crop_right}")
            
            if crop_left < 0:
                crop_left = 0
                crop_right = min(img.width, int(required_crop_width))
                print(f"🔍 DEBUG: Adjusted for left boundary: crop_left: {crop_left}, crop_right: {crop_right}")
        ...
        if ENHANCED_PROCESSING_AVAILABLE and current_processor.enhanced_face_detector and current_processor.ENABLE_ENHANCED_FACE_DETECTION:
            ...
            if face_result.primary_face:
                ...
                face_analysis = {
                    ...
                    "head_height_valid": bool(0.70 <= face_data.face_size_ratio <= 0.80),
                    "horizontally_centered": True,
